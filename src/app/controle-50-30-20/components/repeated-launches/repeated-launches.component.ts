...
@Component({
  selector: "repeated-launches",
  templateUrl: "repeated-launches.component.html",
  styleUrls: ["repeated-launches.component.scss"],
})
export class RepeatedLaunchesComponent implements OnInit {
  ...
  public launches: RepeatedLaunch[] = [];
  ...
  private async loadRepeatedLaunches() {
    let user = this.storageService.getLocalUser();
    if (!user) {
      return;
    }
    this.launches = await this.controleService
      .findRepeatedLaunchesByUserIdAndType(user.accountId)
      .toPromise();
  ...
  remove(launch: RepeatedLaunch, subItems) {
    ...
    dialogRef.afterClosed().subscribe((res) => {
      if (res) {
        this.controleService
          .removeRepeatedLaunch(launch._id)
          .subscribe((res) => {
